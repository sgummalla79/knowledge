...
multi-tenant one (organizations/users-with-roles/embedding_models/sources/categories/shelves/
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("created_by", UUID(as_uuid=True), nullable=True),  # fk -> users, added below
    ...
    op.create_table(
        ...
        sa.Column("last_modified_by", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("last_modified_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    ...
    op.create_foreign_key("organizations_last_modified_by_fkey", "organizations", "users", ["last_modified_by"], ["id"])
    ...
    op.execute(
        ...
        create policy shelf_gated_read on documents
          as restrictive
          for select using (
            exists (select 1 from users u where u.id = current_setting('app.user_id')::uuid and u.role = 'admin')
            or exists (
