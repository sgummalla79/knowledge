...
def test_allows_public_ip():
    with patch("api.infrastructure.web.url_safety.socket.getaddrinfo", return_value=_addrinfo("93.184.216.34")):
...
def test_rejects_non_public_ip(ip):
    with patch("api.infrastructure.web.url_safety.socket.getaddrinfo", return_value=_addrinfo(ip)):
        with pytest.raises(ValidationError) as exc_info:
...
def test_rejects_unresolvable_host():
    ...
    with patch("api.infrastructure.web.url_safety.socket.getaddrinfo", side_effect=socket.gaierror("nope")):
        with pytest.raises(ValidationError):
