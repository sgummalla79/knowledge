...
def test_embed_documents_posts_input_and_returns_vectors_in_order():
    ...
    with patch("api.infrastructure.embeddings.openai_compatible_provider.requests.post") as mock_post:
...
def test_list_models_returns_model_ids():
    ...
    with patch("api.infrastructure.embeddings.openai_compatible_provider.requests.get") as mock_get:
