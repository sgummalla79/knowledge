...
class PasswordIdentityVerifier:
    """Today's IdentityVerifierPort implementation: local email+password. A later SSO/social-login
    swap means writing one new class implementing the same port — nothing else in the application
    ...
    def verify(self, email: str, password: str) -> Identity | None:
        identity = self._repository.get_by_email(email)
