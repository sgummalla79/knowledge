...
def create_application():
    ...
    if dto.auth_method == "oauth_client_credentials":
...
def list_applications():
    ...
    return jsonify([ApplicationResponse.from_entity(application).model_dump(mode="json") for application in applications])


@applications_bp.get("/<uuid:application_id>")
@require_permission("applications:read")
def get_application(application_id: UUID):
    application = _service().get(g.org_id, application_id)
    return jsonify(ApplicationResponse.from_entity(application).model_dump(mode="json"))
...
def update_application(application_id: UUID):
    ...
    application = _service().update(g.org_id, application_id, dto.name, dto.description)
    return jsonify(ApplicationResponse.from_entity(application).model_dump(mode="json"))
...
def rotate_application_key(application_id: UUID):
    service = _service()
    existing = service.get(g.org_id, application_id)
...
def revoke_application(application_id: UUID):
    application = _service().revoke(g.org_id, application_id, g.user_id)
    return jsonify(ApplicationResponse.from_entity(application).model_dump(mode="json"))
