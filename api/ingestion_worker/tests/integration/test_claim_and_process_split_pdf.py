...
def test_worker_processes_an_oversized_pdf_into_multiple_parts(db_session, session_factory):
    ...
    job = ingestion_jobs.create(
        org_id, type="upload", triggered_by=owner.id, payload=pdf_bytes, payload_filename="big.pdf"
    )
    db_session.commit()
    ...
    worker = IngestionJobWorker(session_factory=session_factory, pdf_splitter=splitter)
    with patch(
...
def test_worker_marks_indexed_with_partial_success_when_one_part_fails(db_session, session_factory):
    owner = _owner(db_session)
    org_id = seed_active_embedding_provider(
