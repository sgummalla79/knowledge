"""Regression coverage for a real incident: after EmbeddingSettingsService.update() resizes
...
Every other integration test configures embedding_settings via EmbeddingSettingsRepository.upsert()
directly, bypassing EmbeddingSettingsService.update() entirely — so none of them ever exercised
resize_embedding_column() against a real database. That gap is exactly how this shipped: the ORM's
Chunk.embedding column used to be declared Vector(EMBEDDING_DIM) — a Python-side constant fixed at
process-import time — so resize_embedding_column()'s raw `ALTER TABLE ... TYPE vector(N)` correctly
changed the live Postgres column, but pgvector's client-side bind_processor kept validating inserts
against the stale compiled-in dimension (768) for the lifetime of the process. A production
container restart between saving 1024-dim Voyage settings and the next upload silently reverted
enforcement back to 768, rejecting perfectly valid 1024-dim vectors with "expected 768 dimensions,
not 1024" even though the real column was already vector(1024). Fixed by making the ORM column
dimensionless (Vector()) and deferring all enforcement to Postgres's own column constraint, which
is always accurate regardless of process lifetime.
"""
...
def restore_embedding_dim(db_session):
    ...
    # EmbeddingSettingsService.update() only ever allows a dimension change while chunks is empty).
...
def test_resize_to_non_default_dimension_then_real_insert_succeeds(db_session, restore_embedding_dim):
    ...
    provider_settings_repo.set_enabled("voyage", True)
    db_session.commit()
    ...
    with patch(
        ...
        embedding_settings_service.update(
            "voyage", "voyage-4-lite", "test-key", new_dimension, 800, 100, None
        )
    db_session.commit()
