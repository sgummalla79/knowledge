...
def test_create_category_returns_201_with_location_header(client):
    category = _category()
    with patch("app.presentation.routes.categories.CategoryService.create_category", return_value=category):
...
def test_create_category_duplicate_slug_returns_409(client):
    with patch(
        "app.presentation.routes.categories.CategoryService.create_category",
...
def test_list_categories_returns_all(client):
    ...
    with patch(
        "app.presentation.routes.categories.CategoryService.list_categories",
...
def test_get_missing_category_returns_structured_404(client):
    with patch(
        "app.presentation.routes.categories.CategoryService.get_category",
...
def test_update_category_returns_updated_category(client):
    ...
    with patch("app.presentation.routes.categories.CategoryService.update_category", return_value=category):
...
def test_update_missing_category_returns_structured_404(client):
    with patch(
        "app.presentation.routes.categories.CategoryService.update_category",
        side_effect=NotFoundError("category_not_found", "Category not found.")
    ):
        response = client.patch(
...
def test_delete_category_returns_204(client):
    with patch("app.presentation.routes.categories.CategoryService.delete_category", return_value=None):
...
def test_delete_missing_category_returns_structured_404(client):
    with patch(
        "app.presentation.routes.categories.CategoryService.delete_category",
        side_effect=NotFoundError("category_not_found", "Category not found.")
    ):
        response = client.delete(f"/categories/{uuid4()}")
