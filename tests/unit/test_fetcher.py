...
def test_sends_the_configured_user_agent():
    ...
    with patch("app.infrastructure.web.fetcher.requests.get", return_value=_response(200)) as mock_get:
...
def test_http_error_response_raises_clean_validation_error_not_raw_http_error():
    ...
    fetcher = WebPageFetcher()
    with patch("app.infrastructure.web.fetcher.requests.get", return_value=_response(403)):
...
def test_network_error_raises_validation_error():
    fetcher = WebPageFetcher()
    with patch("app.infrastructure.web.fetcher.requests.get", side_effect=requests.ConnectionError("boom")):
        with pytest.raises(ValidationError) as exc_info:
            fetcher.fetch("https://example.com")
...
def test_follows_redirect_to_final_url():
    ...
    with patch("app.infrastructure.web.fetcher.requests.get", side_effect=responses):
...
def test_js_shell_triggers_playwright_fallback():
    ...
    with patch("app.infrastructure.web.fetcher.requests.get", return_value=shell_response):
        with patch("app.infrastructure.web.fetcher.sync_playwright") as mock_sync_playwright:
