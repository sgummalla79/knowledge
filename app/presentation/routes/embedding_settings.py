...
def list_embedding_settings():
    statuses = _service().list_status(container.get_default_org_id())
...
def get_embedding_settings(provider: str):
    status = _service().get_status(container.get_default_org_id(), provider)
...
def update_embedding_settings(provider: str):
    ...
    if api_key is None:
        ...
        existing = EmbeddingProviderSettingsRepository(get_session()).get(container.get_default_org_id(), provider)
...
def enable_embedding_provider(provider: str):
    status = _service().enable(container.get_default_org_id(), provider)
    return jsonify(EmbeddingProviderConfigResponse.from_status(status).model_dump(mode="json"))


@embedding_settings_bp.post("/embedding-settings/<provider>/disable")
def disable_embedding_provider(provider: str):
    status = _service().disable(container.get_default_org_id(), provider)
    return jsonify(EmbeddingProviderConfigResponse.from_status(status).model_dump(mode="json"))
