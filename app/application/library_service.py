...
class LibraryService:
    ...
    def create_library(self, name: str, description: str | None) -> Category:
    ...
    def get_library(self, library_id: UUID) -> Category:
        library = self._repository.get(library_id)
        if library is None:
            raise NotFoundError(error_codes.LIBRARY_NOT_FOUND, "Library not found.")
        return library

    def update_library(self, library_id: UUID, name: str, description: str | None) -> Category:
    ...
    def list_libraries(self, limit: int, offset: int, sort: str) -> tuple[list[Category], int]:
