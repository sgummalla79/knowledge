...
import type { Application, ApplicationAuthMethod, ApplicationWithClientSecret } from '../api/types'
...
export function ApplicationCreatePage() {
  ...
  const [authMethod, setAuthMethod] = useState<ApplicationAuthMethod>('oauth_client_credentials')
  ...
  const canSubmit =
    name.trim().length > 0 &&
    (authMethod === 'oauth_client_credentials' ? executeAsIdentityId.length > 0 : redirectUris.length > 0)
    ...
    <div>
      ...
      <form id="application-form" onSubmit={handleSubmit} className="flex max-w-4xl flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="flex flex-col gap-4">
            ...
            {authMethod === 'oauth_client_credentials' ? (
              <div>
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  This application inherits exactly what this member's profile grants, and stays in sync if that
          ...
          <div>
            <span className="mb-2 block text-sm text-foreground">Access</span>
